...
export const FolhaPontoPrint: React.FC<FolhaPontoPrintProps> = ({
  ...
  const getRoleLabel = (role?: string) => {
    ...
    switch (role) {
      ...
      case "captador": return "Captador / Angariador";
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto print:absolute print:inset-0 print:bg-white print:p-0 print:m-0 print:backdrop-blur-none">
      ...
      {/* Main Container */}
      <div id="print-area-wrapper" className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl border border-slate-250 flex flex-col h-[90vh] print:h-auto print:max-w-none print:shadow-none print:border-none print:rounded-none">
        ...
        <div className="no-print p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between rounded-t-2xl">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold transition shadow-sm"
            >
        ...
        <div className="flex-1 overflow-y-auto p-8 font-sans text-slate-800 print:overflow-visible print:p-4 print:text-black">
          ...
          <div className="border border-black p-4 mb-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-3 border-b border-black gap-4 mb-3">
              <div>
                <h1 className="text-sm font-black tracking-tight uppercase text-black">
                  {companySettings?.name || "IMOBILIÁRIA CONECTADA"}
                </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
              <div className="border border-dotted border-slate-400 p-2 rounded">
                ...
                <span className="font-extrabold text-slate-800 text-[11px]">{collaborator?.displayName || collaborator?.email || "Não informado"}</span>
              </div>
              <div className="border border-dotted border-slate-400 p-2 rounded">
              ...
              <div className="border border-dotted border-slate-400 p-2 rounded">
                ...
                <span className="font-bold text-slate-800 text-[11px]">{getRoleLabel(collaborator?.role)}</span>
              </div>
              <div className="border border-dotted border-slate-400 p-2 rounded">
                ...
                {diasDoMes.map(dia => {
                  ...
                  return (
                    <tr key={dia} className={`h-7 hover:bg-slate-50/55 ${isWeekend ? "bg-slate-50" : ""}`}>
                      ...
                      <td className={`py-1 px-1 border-r border-black text-center font-bold font-mono ${reg.horasExtras && reg.horasExtras < 0 ? "text-amber-700 print:text-black" : "text-emerald-850 print:text-black"}`}>
                        {saldoStr}
                      </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border border-black p-3 mb-6 bg-slate-50 print:bg-white text-xs">
            <div>
              ...
              <div className="space-y-1.5 font-mono text-[11px] text-slate-800">
                ...
                <div className="flex justify-between border-t border-dashed border-slate-300 pt-1">
                  ...
                  <span className="font-bold text-indigo-700 print:text-black">+{formatMinutesToHHMM(totalExcedente)}h</span>
                </div>
                <div className="flex justify-between">
                  <span>Banco Negativo Período (Faltas/Atrasos):</span>
                  <span className="font-bold text-amber-600 print:text-black">-{formatMinutesToHHMM(totalDeficit)}h</span>
                </div>
                <div className="flex justify-between border-t border-slate-400 pt-1.5 text-xs font-black">
                  <span>SALDO LÍQUIDO ACUMULADO:</span>
                  <span className={`font-black ${totalSaldoNet >= 0 ? "text-emerald-700" : "text-rose-600"} print:text-black`}>
          ...
          <div className="mt-8 pt-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-center text-xs">
              <div className="flex flex-col items-center">
                ...
                <span className="font-bold uppercase text-black">{collaborator?.displayName || collaborator?.email || "COLABORADOR"}</span>
