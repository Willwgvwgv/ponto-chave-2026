...
export default async function handler(req: any, res: any) {
  ...
  let blocoComparativo = "";
  if (tipo === "saida" && comparativoEntrada) {
    const ressalvasEntradaTexto = Array.isArray(comparativoEntrada.ressalvasEntrada) && comparativoEntrada.ressalvasEntrada.length > 0
      ? comparativoEntrada.ressalvasEntrada.map((r: string) => `- ${r}`).join("\n")
      : "Nenhuma ressalva registrada na vistoria de entrada.";

    blocoComparativo = `

DADOS DA VISTORIA DE ENTRADA DESTE MESMO IMÓVEL (para comparação):
- Ressalvas registradas na entrada:
${ressalvasEntradaTexto}
- Observações livres registradas na entrada: """${comparativoEntrada.descricaoGeralEntrada || "(nenhuma)"}"""
${comparativoEntrada.textoLaudoEntrada ? `- Texto do laudo de entrada já gerado (use como referência do estado declarado na entrada): """${comparativoEntrada.textoLaudoEntrada}"""` : ""}

INSTRUÇÃO DE COMPARAÇÃO (muito importante):
Compare cada ressalva/observação da SAÍDA (acima) com o que consta na ENTRADA (acima):
- Se um problema da saída também aparece na entrada (mesmo cômodo/item, mesma natureza), ele é PRÉ-EXISTENTE — declare isso explicitamente e NÃO impute responsabilidade ao locatário por ele.
- Se um problema da saída NÃO aparece em lugar nenhum da entrada, ele é NOVO — pode ser atribuído a dano ocorrido durante a locação, mencionando a comparação com o laudo de entrada como prova disso.
- Nunca presuma que algo é novo só porque não foi mencionado explicitamente — se a entrada não tem informação suficiente sobre aquele ponto específico, diga isso (ex: "não há registro específico na vistoria de entrada sobre este item") em vez de concluir que é dano novo.`;
  }
...
5. ${tipo === "saida" && comparativoEntrada
    ? "Como os dados da vistoria de entrada foram fornecidos acima, faça a comparação de verdade item a item (siga a INSTRUÇÃO DE COMPARAÇÃO) — não apenas mencione que a comparação deveria ser feita depois."
    : "Se for uma vistoria de SAÍDA, inclua uma cláusula final indicando que cabe à imobiliária/locador comparar este laudo com o laudo de entrada para apurar eventuais danos e responsabilidades."}
